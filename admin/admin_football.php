...
<body>
    ...
    if (isset($_POST['add'])) {
        if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            echo "<script>alert('Invalid request.'); window.location.href='admin_football.php';</script>";
            exit();
        }
        ...
        $brand         = $_POST['brand'] ?? '';
        $category      = $_POST['category'] ?? 'football';
        $qty           = (int)($_POST['qty'] ?? 0);
        ...
        if (!isset($_FILES['product_image']) || $_FILES['product_image']['error'] !== UPLOAD_ERR_OK) {
            echo "<script>alert('Image upload failed.'); window.location.href='admin_football.php';</script>";
            exit();
        }
        ...
        if ($size > 30 * 1024 * 1024) { // keep your original 30GB logic but practical 30MB
            echo "<script>alert('File too large.'); window.location.href='admin_football.php';</script>";
            exit();
        }
	...
	if (!move_uploaded_file($temp, $targetPath)) {
        ...
        exit(header("Location: admin_football.php"));
    }
    ?>
    ...
    <div id="rightcontent" style="position:absolute; top:10%;">
        <div class="alert alert-info"><center><h2>Foot Ball</h2></center></div>
        <br />
